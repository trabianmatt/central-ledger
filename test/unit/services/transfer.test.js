...
Test('Transfer Service tests', serviceTest => {
  ...
  serviceTest.test('saveTransferPrepared should', preparedTest => {
    ...
    preparedTest.test('save transfer record to read model', assert => {
      ...
      AccountsModel.getByName.withArgs(dfsp1Account.name).returns(Promise.resolve(dfsp1Account))
      AccountsModel.getByName.withArgs(dfsp2Account.name).returns(Promise.resolve(dfsp2Account))
